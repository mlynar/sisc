function [ws,r, Cmax2, spk] = N_temporalMP(y,B,nonnegative,maxiter,mindelta,deadzone,spkTrs)
    ...
    %TODO: BUG - line 192 rg gets negative if mindelta = 0;
    ...
    scales = zeros(1, size(B, 3));
    for i = 1:length(scales)
        scales(i) = 1 / sqrt(sum(sum(B(:,:,i).^2)));
        B(:,:,i) = B(:,:,i) * scales(i);
    end
    ...
    padsize = size(B,2);
    nch = size(B,1);
    y = [zeros(nch,padsize),y,zeros(nch,padsize)];
    
    %Compute the convolution of y with each element in B
    C = myconvnfft2(B(:,end:-1:1,:),y);
    ...
    %functions
    P = zeros(size(B,3),size(B,2)*2-1,size(B,3));
    for ii = 1:size(B,3)
        P(:,:,ii) = myconvnfft2(B(:,end:-1:1,:), squeeze(B(:,:,ii)));
    end
    ...
    while ii <= maxiter        
        ...
        if abs(themax) < spkTrs
            fprintf('Spike thresholed reached at it %d with %.3f val\n', ii, themax);
            break;
        end
        ...
        rg = maxj + rgdelta;
        delta = C(maxi,maxj)*squeeze(B(:,:,maxi)); 
...
function R = myconvnfft2(B, y)
    nsize = size(B,2)+size(y,2)-1;
    R = zeros(size(B,3), nsize); 
    for i = 1:size(B,1)
        R = R + myconvnfft(squeeze(B(i,:,:))', y(i,:));
    end
end
