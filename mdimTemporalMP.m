function [ws,r, Cmax2] = mdimTemporalMP(y,B,nonnegative,maxiter,mindelta,deadzone, snrTrs, spkTrs, nskip)
    ...
    if nskip > 1 & mod(length(y), nskip) ~= 0
        error('time series length has to be divisible by y');
    end
    ...
    %functions
    if nonnegative
        Cmax = max(C(:, skipinds)); %Cmax - wektor dajacy maksymalna wartosc konwolucji  w kazdym tp - co nskip
    else
        Cmax = max(abs(C(:, skipinds)));
    end
    ...
    while ii <= maxiter        
        ...
        maxj_skip = (maxsqrt-1)*nsqrt + maxoffset; %polozenie wartosci maksymalnej w przerzedzonym szeregu
        ...
        %full - full time series corresponding to - short - sparsified time series
        rg2_short = (rg2 - 1) / nskip + 1;
        rg2_full = rg2(fix(rg2_short) == rg2_short);
        rg2_short = rg2_short(fix(rg2_short) == rg2_short);
        
        if length(rg2_short) ~= length(rg2_full)
            error('Line 229 madafaka!')
        end
...
%         rg2_full
        ...
        %TODO: narazie wylaczam deadzone
        %if deadzone > 0
        ...
        if rg2(end) > length(y) - size(B,2) -1 || ...
           rg2(1) < size(B,2) +1
            C(:,Cmask) = 0;
            Cmax(Cmask(skipinds)) = 0;
        end
        
        %Update relevant parts of Cmax2
        lopos = ceil(rg2_short(1)/nsqrt);
        hipos = ceil(rg2_short(end)/nsqrt);
